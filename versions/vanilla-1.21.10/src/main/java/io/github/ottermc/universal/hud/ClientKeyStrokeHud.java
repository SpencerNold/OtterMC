...
import io.github.ottermc.universal.GLFWKeyboard;
...
public class ClientKeyStrokeHud extends KeyStrokeHud {
    ...
    private void drawKeys(DrawContext context, MinecraftClient mc, boolean clickColor) {
        ...
        KeyObject object;
        // w
        object = new KeyObject(mc.options.forwardKey, getDefaultX() + 21, getDefaultY(), 19, 19);
        object.draw(context, mc, color, clickColor);
        // a
        object = new KeyObject(mc.options.leftKey, getDefaultX(), getDefaultY() + 21, 19, 19);
        object.draw(context, mc, color, clickColor);
        // s
        object = new KeyObject(mc.options.backKey, getDefaultX() + 21, getDefaultY() + 21, 19, 19);
        object.draw(context, mc, color, clickColor);
        // d
        object = new KeyObject(mc.options.rightKey, getDefaultX() + 42, getDefaultY() + 21, 19, 19);
        object.draw(context, mc, color, clickColor);
        // space
        object = new KeyObject(mc.options.jumpKey, getDefaultX(), getDefaultY() + 42, 61, 19);
        object.draw(context, mc, color, clickColor);
    }

    private record KeyObject(KeyBinding binding, int x, int y, int width, int height) {
        ...
        private InputUtil.Key getBoundKey(KeyBinding binding) {
            return (InputUtil.Key) Reflection.getValue("net/minecraft/client/option/KeyBinding", binding, "boundKey");
        }
    }
}
