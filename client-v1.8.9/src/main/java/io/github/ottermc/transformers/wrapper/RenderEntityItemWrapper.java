...
import io.github.ottermc.events.listeners.GetItemScaleListener.GetItemScaleEvent;
	...
	public static void doRender(RenderEntityItem renderer, EntityItem entity, double x, double y, double z, float yaw, float partialTicks) {
		...
		for (int j = 0; j < i; ++j) {
			if (ibakedmodel.isGui3d()) {
				...
				GetItemScaleEvent event = new GetItemScaleEvent(renderer, entity, 0.5f, 0.5f, 0.5f);
				EventBus.fire(event);
				...
				GlStateManager.pushMatrix();
				GetItemScaleEvent event = new GetItemScaleEvent(renderer, entity, 1.0f, 1.0f, 1.0f);
				EventBus.fire(event);
				GlStateManager.scale(event.getX(), event.getY(), event.getZ());
				ibakedmodel.getItemCameraTransforms().applyTransform(ItemCameraTransforms.TransformType.GROUND);
				getItemRenderer().renderItem(itemstack, ibakedmodel);
