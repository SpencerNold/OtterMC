...
public class Profiler {
    ...
    public static void main(String[] args) {
        ...
        if (element == null)
            System.exit(4);
        int exitCode = addToProfile(mcDir, element, args[1]);
        if (exitCode != 0)
    ...
    private static int addToProfile(File mcDir, JsonElement element, String javaPath) {
        if (!element.isJsonObject())
            return 1;
        element = element.getAsJsonObject().get("profiles");
        if (!element.isJsonObject())
            return 2;
        String name = "ottermc-client";
        ...
        versionObject.add("name", new JsonPrimitive("OtterMC"));
        versionObject.add("type", new JsonPrimitive("custom"));
        versionObject.add("javaDir", new JsonPrimitive(String.join(File.separator, mcDir.getAbsolutePath(), "jre", "21", javaPath)));
